...
class AIChat(commands.Cog):
    ...
    # ── /set-ai ───────────────────────────────────────────────────
    # 修正①: type の型を app_commands.Choice[str] に変更
    ...
    async def set_ai(self, interaction: discord.Interaction, type: app_commands.Choice[str]):
        self.channel_settings[interaction.channel_id] = type.value
        self.history[interaction.channel_id].clear()
        label = AI_CHOICE_LABEL.get(type.value, type.value)
    ...
    # ── フォールバック付き呼び出し ────────────────────────────────
    # 修正②: primary が "or:" で始まることを確認してからスライス
    # 修正③: tried カウントを fallbacks リストの位置から正しく計算
    ...
    async def _call_with_fallback(
        ...
        # primary が OR モデルであることを保証
        if not primary.startswith("or:"):
            raise ValueError(f"_call_with_fallback に非ORモデルが渡されました: {primary}")
        ...
        # fallbacks: primary を除いた残りの OR モデル
        fallbacks = [v for v in OR_FALLBACK_ORDER if v != primary]
        # 修正③: total = primary(1) + fallbacks + DuckDuckGo(1)
        total = 1 + len(fallbacks) + 1

        status_msg: discord.Message = await origin.reply(
            f"⚠️ **429 レート制限** が発生しました。他のモデルに順番に切り替えます... (1/{total})"
        )

        for tried, value in enumerate(fallbacks, start=2):
        ...
        await status_msg.edit(content=f"🦆 **{ddg_label}** を試行中... ({total}/{total})")
